...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Skip if expiry_time is NULL
        if (is_null($expiry_time)) {
            continue;
        }

        // Parse expiry_time as DateTime
        $expiry_time = DateTime::createFromFormat("Y-m-d H:i:s", $expiry_time);

        // Ensure expiry_time is valid before comparison
        if ($expiry_time && $expiry_time <= $current_time) { // Compare both date and time
