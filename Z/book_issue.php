...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_query = "SELECT avail_quantity FROM book WHERE access_no = ? AND id = ?";
    ...
    if ($book && $book['avail_quantity'] > 0) {
        // Insert book issue record
        $issue_query = "INSERT INTO book_issue (book_id, access_no, book_name, author_name, receiver, student_name, class, year, student_id, contact, staff_name, department, issued_date, due_date, return_date, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($issue_query);
        $stmt->bind_param("isssssssssssssss", $book_id, $access_no, $book_name, $author_name, $receiver, $student_name, $class, $year, $student_id, $contact, $staff_name, $department, $issued_date, $due_date, $return_date, $status);
        $stmt->execute();

        // Update available quantity
        $update_query = "UPDATE book SET avail_quantity = avail_quantity - 1 WHERE access_no = ? AND id = ?";
...
<body>
    ...
    <body>
        ...
        <!-- JavaScript Moved Here (Before </body>) -->
        <script>
            ...
            // Call toggleFields() and updateYearOptions() when the page loads
